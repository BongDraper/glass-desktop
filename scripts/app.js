        ...
        function readLocalProjectsRaw() {
            try {
                return localStorage.getItem(PROJECTS_STORAGE_KEY);
            } catch (err) {
                console.warn('Unable to read local project storage.', err);
                return null;
            }
        }

        function writeLocalProjectsSafe(serializedProjects) {
            try {
                localStorage.setItem(PROJECTS_STORAGE_KEY, serializedProjects);
                return true;
            } catch (err) {
                console.warn('Unable to persist local project storage.', err);
                showStorageWarning('Could not save more local edits (browser storage full/restricted). Your current change is visible now, but may not survive refresh. Use Export Project Data and commit it to the repo.');
                return false;
            }
        }

        function clearLocalProjectsSafe() {
            try {
                localStorage.removeItem(PROJECTS_STORAGE_KEY);
            } catch (err) {
                console.warn('Unable to clear local project storage.', err);
            }
        }
        ...
        function loadProjects() {
            const savedProjects = readLocalProjectsRaw();
            ...
            try {
                ...
                if (JSON.stringify(parsed) !== JSON.stringify(sanitizedProjects)) {
                    writeLocalProjectsSafe(JSON.stringify(sanitizedProjects));
                }
                return sanitizedProjects;
            } catch (err) {
                console.warn('Ignoring invalid saved projects payload.', err);
                clearLocalProjectsSafe();
            }
        ...
        function persistProjects() {
            ...
            writeLocalProjectsSafe(JSON.stringify(projects));
        }
...
`;
        ...
        function closeWindow(id) { const el = document.getElementById(id); if (el) el.style.display = 'none'; }
        function openWindow(id) { const el = document.getElementById(id); if (!el) return; el.style.display = 'flex'; focusWindow(id); }
        function focusWindow(id) { const el = document.getElementById(id); if (!el) return; document.querySelectorAll('.window').forEach(w => w.style.zIndex = 100); el.style.zIndex = 200; }
        ...
        if (projectIconInput) {
            projectIconInput.onchange = (e) => {
                ...
                reader.onload = (ev) => {
                    ...
                    if (prev) {
                        prev.style.backgroundImage = `url(${pendingIconData})`;
                        prev.innerText = '';
                    }
                };
        ...
        async function checkPass() {
            ...
            let savedHash = null;
            try { savedHash = localStorage.getItem(AUTH_KEY); } catch (err) { console.warn('Unable to read auth storage.', err); }
            const incomingHash = await hashPassword(password);

            if (!savedHash) {
                try { localStorage.setItem(AUTH_KEY, incomingHash); } catch (err) { console.warn('Unable to save auth storage.', err); }
        ...
        const cpTrigger = document.getElementById('cpTrigger');
        if (cpTrigger) cpTrigger.onclick = () => promptLogin();

        const loginSubmit = document.getElementById('loginSubmit');
        if (loginSubmit) loginSubmit.onclick = () => checkPass();
        ...
        if (wallpaperInput) {
            wallpaperInput.onchange = (e) => {
                ...
                if (!file) return;
                const reader = new FileReader();
                reader.onload = (ev) => { currentWallpaper = ev.target.result; const desktop = document.getElementById('desktop'); if (desktop) desktop.style.backgroundImage = `url(${currentWallpaper})`; };
